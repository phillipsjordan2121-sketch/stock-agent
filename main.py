...
PROMPT = """You are a short-term trading analyst covering the Tech, AI, and Semiconductor sector. Analyze the news and analyst actions from the last 24 hours provided below. Identify short-term buy opportunities (1â5 day holds) only. Focus on: AI chip plays (Nvidia, AMD, TSMC, Broadcom, Marvell, etc.), AI software with strong momentum, semiconductor equipment, and cloud infrastructure.

For each opportunity, output exactly this format:
TICKER | Company | Confidence: X% | Hold: XâX days | Catalyst: [one sentence citing the specific news] | Edge: [why the market might not have fully priced this in yet] | Invalidation: [1â2 sentences on what would signal early exit within the hold window]

Requirements:
â Every pick must be anchored to a specific catalyst from the provided data â no generic sector tailwinds
â Confidence % should reflect both catalyst strength and how cleanly the setup fits a 1â5 day hold
â The Edge field must identify a concrete mispricing reason: delayed reaction, overlooked secondary beneficiary, analyst revision lag, low retail awareness, etc. â not "market hasn't caught up yet" as a standalone claim; name the specific mechanism
â Rank picks by confidence, highest first
â If a catalyst is strong but the hold window extends beyond 5 days, exclude it
â If two tickers share the same catalyst, include both only if each has a distinct, independently justifiable Edge â state what makes each Edge different from the other
â For each pick, also include a brief note (1â2 sentences) on what would invalidate the thesis within the hold window (conditions that would signal early exit)
...
def fetch_market_news():
    ...
    for a in recent[:40]:  # cap at 40 articles to stay within token budget
        lines.append(f"[NEWS] {a.get('headline', '')} â {a.get('summary', '')[:200]}")
    return lines
...
def fetch_company_news(ticker):
    ...
    try:
        ...
        for a in articles[:3]:  # top 3 per ticker
            lines.append(f"[{ticker} NEWS] {a.get('headline', '')} â {a.get('summary', '')[:150]}")
        return lines
    except Exception:
        return []
...
def format_email_html(picks: list[str]) -> str:
    ...
    for pick in picks:
        ...
        ticker   = parts[0] if len(parts) > 0 else "â"
        company  = parts[1] if len(parts) > 1 else "â"
        conf     = parts[2] if len(parts) > 2 else "â"
        hold     = parts[3] if len(parts) > 3 else "â"
        catalyst = parts[4] if len(parts) > 4 else "â"
        edge     = parts[5] if len(parts) > 5 else "â"
        invalid  = parts[6] if len(parts) > 6 else "â"
    ...
    <html><body style="margin:0;padding:0;background:#f9fafb;font-family:-apple-system,BlinkMacSystemFont,'Segoe UI',sans-serif;">
      <div style="max-width:600px;margin:0 auto;padding:32px 16px;">
        <div style="margin-bottom:24px;">
          <div style="font-size:11px;font-weight:700;text-transform:uppercase;letter-spacing:0.1em;color:#6b7280;margin-bottom:6px;">Tech Â· AI Â· Semiconductors</div>
          <div style="font-size:26px;font-weight:800;color:#111;letter-spacing:-0.5px;">Morning Picks</div>
          <div style="font-size:14px;color:#9ca3af;margin-top:4px;">{today} Â· {len(picks)} pick{"s" if len(picks) != 1 else ""} above {CONFIDENCE_THRESHOLD}% confidence</div>
        </div>
        {rows}
...
def send_email(picks: list[str]):
    subject = f"ð {len(picks)} High-Confidence Pick{'s' if len(picks) != 1 else ''} â {datetime.now().strftime('%b %d')}"
